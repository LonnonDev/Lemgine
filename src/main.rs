...
impl Rendering for WindowDrawer {
    fn render_cube(&mut self, variables: Vec<VecTuple>) -> Vec<VecTuple> {
        ...
        target.draw(&self.vertex_buffer, &indices, &self.program, &uniform!{ x: variables[0].unwrap(), y: variables[1].unwrap() }, &Default::default()).unwrap();
...
impl Input for WindowDrawer {
    fn get_movement(&mut self, mut variables: Vec<VecTuple>, _: DeviceId, input: KeyboardInput, _: bool) -> Vec<VecTuple> {
        if input.virtual_keycode == Some(VirtualKeyCode::W) && input.state == ElementState::Pressed {
            variables[1] = VecTuple::F32(variables[1].unwrap() + (variables[1].unwrap() + 10.0)/self.size.width);
            ...
            variables[0] = VecTuple::F32(variables[0].unwrap() + (variables[0].unwrap() + 10.0)/self.size.width);
...
fn main() {
    ...
    let variables = vec![VecTuple::U32(0), VecTuple::U32(0)];
